...
router.post('/', authUser, sanitizeBody, requireAdmin, async (req, res, next) => {
    try {
        ...
        const newPerson = new Student(req.sanitizedBody)
...
router.get('/:id', authUser, requireAdmin, async (req, res, next) => {
    try {
        ...
        const person = await Student.findById(req.params.id).populate('gifts')
...
const update = (overwrite = false) => async (req, res, next) => {
    try {
        await validateId(req.params.id)
        const person = await Student.findByIdAndUpdate(
        req.params.id,
        req.sanitizedBody,
        {
            new: true,
...
router.delete('/:id', authUser, requireAdmin, async (req, res, next) => {
    try {
        await validateId(req.params.id)
        const person = await Student.findByIdAndRemove(req.params.id)
        res.send({ data: person })
    } catch (err) {
        next(err)
    }
